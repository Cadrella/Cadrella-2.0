function clickOnCatalog(catalog_name) {

    if(styleOfWall) styleOfWall.remove();

    whichCatalog = catalog_name;
    ...
    function addProductFromCatalog(product, number_in_catalog) {
        if(product.catalog_name != catalog_name) return;
        
        const img = document.createElement("img");
            img.className = "product_img";
            img.src = product.product_image;

            while(i > number_in_catalog) {
                number_in_catalog += 4;
            }

            if(number_in_catalog == i) {
                first_row.appendChild(img);
                img.setAttribute('data-column', '1');
            }
            else if(number_in_catalog == i+1) {
                second_row.appendChild(img);
                img.setAttribute('data-column', '2');
            }
            else if(number_in_catalog == i+2) {
                third_row.appendChild(img);
                img.setAttribute('data-column', '3');
            }
            else if(number_in_catalog == i+3) {
    ...
    for (i; i < products_data.length; i+=4) {
        if(products_data[i]){
            addProductFromCatalog(products_data[i], products_data[i].product_number_in_catalog -1);
        }
        if(products_data[i+1]){
            addProductFromCatalog(products_data[i+1], products_data[i+1].product_number_in_catalog -1);
        }
        if(products_data[i+2]){
            addProductFromCatalog(products_data[i+2], products_data[i+2].product_number_in_catalog -1);
        }
        if(products_data[i+3]){
            addProductFromCatalog(products_data[i+3], products_data[i+3].product_number_in_catalog -1);
        }
    }
